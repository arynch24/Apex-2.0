...
export const BackgroundCells = ({
  ...
  return (
    <div
      className={cn("relative h-screen flex justify-center overflow-hidden", className)}>
      <BackgroundCellCore />
      {children && (
  ...
  return (
    <div
      ...
      <div className="absolute h-[20rem] inset-y-0 overflow-hidden">
        <div
          className="absolute h-full w-full pointer-events-none -bottom-2 z-40 bg-slate-950 [mask-image:linear-gradient(to_bottom,transparent,black)]" />
        <div
      ...
      {matrix.map((row, rowIdx) => (
        <div
          ...
          {row.map((column, colIdx) => {
            ...
            useEffect(() => {
              if (clickedCell) {
                const distance = Math.sqrt(Math.pow(clickedCell[0] - rowIdx, 2) +
